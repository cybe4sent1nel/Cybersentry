...
Agents are the core of Cybersentry. An agent uses Large Language Models (LLMs), configured with instructions and tools to perform specialized cybersecurity tasks. Each agent is defined in its own `.py` file in `src/cai/agents` and optimized for specific security domains.

## Available Agents

Cybersentry provides a comprehensive suite of specialized agents for different cybersecurity scenarios:
...
```bash
# Launch Cybersentry with a specific agent
Cybersentry_AGENT_TYPE=redteam_agent cai

# Launch with custom model
Cybersentry_AGENT_TYPE=bug_bounter_agent Cybersentry_MODEL=Neural x SEC cai
...
Cybersentry>/agent info redteam_agent
```
...
Cybersentry> Scan https://target.com for vulnerabilities

# 2. Switch to exploitation
Cybersentry>/agent redteam_agent  
Cybersentry> Exploit the SQL injection found at /login

# 3. Post-exploitation analysis
Cybersentry>/agent dfir_agent
Cybersentry> Analyze the logs to understand the attack surface
```
...
Cybersentry>/agent subghz_sdr_agent
Cybersentry> Analyze the 433MHz signals from the device

# 2. Firmware analysis
Cybersentry>/agent reverse_engineering_agent
Cybersentry> Extract and analyze the firmware from dump.bin

# 3. Memory analysis if device captured
Cybersentry>/agent memory_analysis_agent
Cybersentry> Analyze the memory dump for secrets
```
...
Cybersentry>/agent network_traffic_analyzer
Cybersentry> Analyze capture.pcap for suspicious activity

# 2. Forensic investigation
Cybersentry>/agent dfir_agent
Cybersentry> Investigate the compromised host logs

# 3. Defensive recommendations
Cybersentry>/agent blueteam_agent
Cybersentry> Provide mitigation strategies based on findings
```
...
network_agent = Agent(
    ...
    model=OpenAIChatCompletionsModel(
        model=os.getenv('Cybersentry_MODEL', "qwen2.5:72b"),
        openai_client=AsyncOpenAI(),
    )
)
lead_agent = Agent(
...
```bash
Cybersentry_STREAM=true cai
```
**2. Enable tracing for debugging:**
```bash
Cybersentry_TRACING=true cai
```
...
Cybersentry> Test https://target.com for vulnerabilities

# ❌ Bad: Using reverse engineering agent for web testing
Cybersentry_AGENT_TYPE=reverse_engineering_agent cai
Cybersentry> Test https://target.com for vulnerabilities
```
...
```bash
Cybersentry>/agent bug_bounter_agent
Cybersentry> Find vulnerabilities in the web app
# ... agent finds SQL injection ...

Cybersentry>/agent redteam_agent
Cybersentry> Exploit the SQL injection to gain access
# ... successful exploitation ...

Cybersentry>/agent dfir_agent
Cybersentry> Analyze what data was exposed during the test
```
...
Cybersentry_PRICE_LIMIT="5.00" Cybersentry_MAX_TURNS=50 cai
```
...
# In future session
Cybersentry>/load logs/logname.jsonl
```
