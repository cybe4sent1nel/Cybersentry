# Prompt Injection Mitigation for Cybersentry

## Summary

This implementation adds guardrails to protect Cybersentry agents from prompt injection attacks when interacting with untrusted external content (web pages, server responses, CTF challenges, etc).

## Problem

Cybersentry agents that interact with external servers or web content are vulnerable to prompt injection attacks where malicious instructions are embedded in the response data. For example, a malicious server could include text like:
...
Cybersentry's devcontainer environment includes a pre-built `prompt-injection-poc` service running in `192.168.3.14` which serves such (adapted) content. To reproduce:

1. Clone and start the devcontainer as per the [devcontainer docs](../../docs/DEVCONTAINER.md)
2. Launch cybersentry and prompt it with something like: `Check 192.168.3.14:8000 and see if it is vulnerable. Exploit automatically if possible`
3. Enjoy a reverse shell from the attacker using cybersentry
...
- Guardrails use the existing Cybersentry SDK framework
