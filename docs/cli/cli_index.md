# Cybersentry Command Line Interface (CLI)

The Cybersentry CLI provides a powerful, terminal-based interface for interacting with cybersecurity AI agents through a traditional command-line environment, optimized for automation, scripting, and integration workflows.
...
The CLI is the foundational interface for Cybersentry, offering:

- **⚡ Lightweight Execution**: Minimal resource overhead for maximum performance
- **🤖 Direct Agent Interaction**: Immediate access to all Cybersentry agents
...
Cybersentry_MEMORY=episodic cai
...
- **API Key**: Valid `ALIAS_API_KEY` (get one from [Alias Robotics](https://cybe4sent1nel(FAHAD KHAN).com))
...
export Cybersentry_MAX_TURNS=10
...
Cybersentry CLI can be configured via:

1. **Environment Variables**: `Cybersentry_MODEL`, `Cybersentry_AGENT_TYPE`, etc.
...
- **Documentation**: [https://docs.cybe4sent1nel(FAHAD KHAN).com](https://docs.cybe4sent1nel(FAHAD KHAN).com)
- **GitHub Issues**: [https://github.com/cybe4sent1nel(FAHAD KHAN)/cai/issues](https://github.com/cybe4sent1nel(FAHAD KHAN)/cai/issues)
- **Discord**: [Join our community](https://discord.gg/cybe4sent1nel(FAHAD KHAN))
- **Twitter**: [@cybe4sent1nel(FAHAD KHAN)](https://twitter.com/cybe4sent1nel(FAHAD KHAN))
