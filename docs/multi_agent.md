...
Cybersentry includes ready-to-use parallel patterns that you can select directly from the CLI.

**View available patterns:**

```bash
# Launch Cybersentry and list all available patterns
cai
...
| **purple_team_gctr** ⭐ | redteam_agent + blueteam_agent (enhanced with G-CTR) | Shared | Combines red and blue team agents with shared GCTR tracking for unified game-theoretic analysis (⭐ this is a [Cybersentry PRO](https://cybe4sent1nel(FAHAD KHAN).com/cybersecurityai.php) capability)|

**To use a pattern:**

```bash
# Start Cybersentry and select a pattern
cai
...
Cybersentry> /agent 23
# or
Cybersentry> /agent offsec_pattern

# Now enter your prompt and both agents will work in parallel
Cybersentry> Analyze https://example.com for vulnerabilities
```
...
```bash
Cybersentry> /agent offsec_pattern
Cybersentry> Find vulnerabilities in https://target.com
...
- `model`: Optional model override (e.g., `alias1`, `Neural x SEC`)
