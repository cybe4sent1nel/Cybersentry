...
> This guide shows how to run Cybersentry against Azure-hosted OpenAI's models
...
**Note:** Cybersentry uses the OpenAI SDK style `base_url + /chat/completions`. For Azure, providing the full endpoint above (including `chat/completions?api-version=...`) ensures correct routing.
...
#### 5. Start Cybersentry and select the model
Launch Cybersentry and select the Azure model:

```vbnet
Cybersentry> /model azure/<model-name>
╭─────────────────────────────────────────────────── Model Changed ────────────────────────────────────────────────────╮
...
> ⚠️ Remember: you must select the model each time you start Cybersentry.


> EXTRA configuration:
You can set the variable `Cybersentry_MODEL` to avoid the need for repeated model setup during initialization.
