...
Cybersentry TUI includes powerful advanced features for professional security workflows. This guide covers the key capabilities beyond basic terminal usage.
...
MCP is an open protocol that connects Cybersentry agents to external tools and services, dramatically expanding their capabilities.
...
For detailed instructions on enabling, configuring, and using MCP with Cybersentry, including setup guides, supported servers, security considerations, and practical examples, see the complete [MCP Configuration Guide](../cai/getting-started/MCP.md).
...
Create specialized agents for your unique workflows (requires Cybersentry PRO).
...
```bash
# In .env
Cybersentry_PRICE_LIMIT=50.0       # Stop at $50
```

### Model Selection Strategy

- **Reconnaissance**: Use `Neural x SEC-fast` or `alias1` (fast, cheap)
