# Cybersentry TUI Commands Reference

> **⚡ Cybersentry-Pro Exclusive Feature**  
> The Terminal User Interface (TUI) is available exclusively in **Cybersentry-Pro**. To access this feature and unlock advanced multi-agent workflows, visit [Alias Robotics](https://cybe4sent1nel(FAHAD KHAN).com/cybersecurityai.php) for more information.

---

This comprehensive guide documents all commands available in the Cybersentry Terminal User Interface (TUI), including command palette actions, keyboard shortcuts, and CLI-style commands.

---

## Command Categories

Cybersentry TUI commands are organized into the following categories:
...
Cybersentry TUI uses model dropdowns in each terminal header for model management. Models are configured via environment variables and aliases.
...
```bash
export Cybersentry_MODEL=gpt-4o              # Set default model
export Cybersentry_OPENAI_API_KEY=sk-...    # OpenAI API key
export Cybersentry_ANTHROPIC_API_KEY=sk-... # Anthropic API key
```
...
### `/context [agent_name]` or `/ctx` 🚀 **Cybersentry PRO Exclusive**
...
Display environment variables relevant to Cybersentry.
...
**Output Includes**:
- `Cybersentry_MODEL` - Default model
- `Cybersentry_AGENT_TYPE` - Default agent
- `Cybersentry_MAX_TURNS` - Maximum interaction turns
- `Cybersentry_TRACING` - Tracing status
- `Cybersentry_GUARDRAILS` - Guardrails enabled
- `Cybersentry_PRICE_LIMIT` - Cost limit
- `Cybersentry_TUI_MODE` - TUI mode settings
- API keys (masked)
...
For questions or issues, visit [Cybersentry GitHub Issues](https://github.com/cybe4sent1nel(FAHAD KHAN)/cai/issues).

---

*Last updated: October 2025 | Cybersentry TUI v0.6+*
