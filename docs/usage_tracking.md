# Cybersentry Global Usage Tracking

Cybersentry now includes automatic global usage tracking that persists token usage and costs across all sessions to `$HOME/.cai/usage.json`.

## Features

- **Automatic Tracking**: All LLM interactions are automatically tracked
- **Global Persistence**: Usage data persists across all Cybersentry sessions
...
```bash
export Cybersentry_DISABLE_USAGE_TRACKING=true
```
