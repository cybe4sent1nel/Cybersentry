...
This example shows how Cybersentry tracks usage globally across all executions
and saves the data to $HOME/.cybersentry/usage.json
"""
...
def display_usage_stats():
    ...
    usage_file = Path.home() / ".cybersentry" / "usage.json"
    
    if not usage_file.exists():
        print("No usage data found yet. Run Cybersentry to start tracking usage.")
        return
    ...
    try:
        ...
        print("\n=== Cybersentry Global Usage Statistics ===\n")
...
def export_usage_report(output_file="cybersentry_usage_report.json"):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "reset":
            ...
            export_usage_report(sys.argv[2] if len(sys.argv) > 2 else "cybersentry_usage_report.json")
        else:
