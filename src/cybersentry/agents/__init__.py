"""
CYBERSENTRY agents abstraction layer

CYBERSENTRY abstracts its cybersecurity behavior via agents and agentic patterns.
...
- **\\(D\\) (Decision Mec.y.b.e.r.s.e.n.t.r.y.m):** A decision function \\( D: S \to A \\) where \\( S \\) represents system states, and \\( D \\) determines which agent takes action at any given time.
...
| `Hierarchical` | A top-level agent (e.g., "PlannerAgent") assigns tasks via structured handoffs to spec.y.b.e.r.s.e.n.t.r.y.ed sub-agents. Alternatively, the structure of the agents is harcoded into the agentic pattern with pre-defined handoffs. |
...
from cybersentry.agents.flag_discriminator import flag_discriminator, transfer_to_flag_discriminator
from cybersentry.sdk.agents import Agent
from cybersentry.sdk.agents.handoffs import handoff

# Extend the search path for namespace packages (allows merging)
__path__ = pkgutil.extend_path(__path__, __name__)

# Get model from environment or use default
model = os.environ.get("CYBERSENTRY_MODEL", "cybe4sent1nel0")
...
def get_available_agents() -> Dict[str, Agent]:  # pylint: disable=R0912  # noqa
    ...
    from the cybersentry/agents folder.
    ...
    from cybersentry.agents.patterns import PATTERNS
...
def get_agent_module(agent_name: str) -> str:
    ...
    Returns:
        ...
        is defined (e.g., 'cybersentry.sdk.agents.basic')
    """
    # Try to import all agents from the agents folder
    for _, name, _ in pkgutil.iter_modules(__path__, __name__ + "."):
        try:
...
def get_agent_by_name(agent_name: str, custom_name: str = None, model_override: str = None, agent_id: str = None) -> Agent:
    ...
    from cybersentry.agents.factory import get_agent_factory
    ...
    if hasattr(agent, "model") and hasattr(agent.model, "__class__"):
        try:
            ...
            if model_class.__name__ == "OpenAIChatCompletionsModel":
                ...
                try:
                    from cybersentry.repl.commands.mcp import get_mcp_tools_for_agent
