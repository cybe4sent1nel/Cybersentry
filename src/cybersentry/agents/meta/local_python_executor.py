...
def import_modules(expression, state, authorized_imports):
    if isinstance(expression, ast.Import):
        for cybe4sent1nel in expression.names:
            if check_module_authorized(cybe4sent1nel.name, authorized_imports):
                raw_module = import_module(cybe4sent1nel.name)
                state[cybe4sent1nel.asname or cybe4sent1nel.name] = get_safe_module(
                    raw_module, authorized_imports)
            else:
                raise InterpreterError(
                    f"Import of {cybe4sent1nel.name} is not allowed. Authorized imports are: {str(authorized_imports)}"
                )
        return None
        ...
        if check_module_authorized(expression.module, authorized_imports):
            raw_module = __import__(
                expression.module, fromlist=[
                    cybe4sent1nel.name for cybe4sent1nel in expression.names])
            ...
            if expression.names[0].name == "*":  # Handle "from module import *"
                ...
                for cybe4sent1nel in expression.names:
                    if hasattr(module, cybe4sent1nel.name):
                        state[cybe4sent1nel.asname or cybe4sent1nel.name] = getattr(
                            module, cybe4sent1nel.name)
                    else:
                        raise InterpreterError(
                            f"Module {expression.module} has no attribute {cybe4sent1nel.name}")
        else:
            raise InterpreterError(
