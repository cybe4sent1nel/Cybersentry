...
interacting with CYBERSENTRY agents.
...
---------------------
    ...
    Optional:
        ...
        CYBERSENTRY_MODEL: Model to use for agents
            (default: "cybe4sent1nel0")
        CYBERSENTRY_DEBUG: Set debug output level (default: "1")
            - 0: Only tool outputs
            - 1: Verbose debug output
            - 2: CLI debug output
        CYBERSENTRY_BRIEF: Enable/disable brief output mode (default: "false")
        CYBERSENTRY_MAX_TURNS: Maximum number of turns for
        ...
        CYBERSENTRY_MEMORY_OFFLINE: Enable/disable offline memory
            (default: "false")
        CYBERSENTRY_ENV_CONTEXT: Add enviroment context, dirs and
            current env available (default: "true")
        CYBERSENTRY_MEMORY_ONLINE_INTERVAL: Number of turns between
        ...
        CYBERSENTRY_SUPPORT_MODEL: Model to use for the support agent
            (default: "o3-mini")
        CYBERSENTRY_SUPPORT_INTERVAL: Number of turns between support agent
            executions (default: "5")
        CYBERSENTRY_STREAM: Enable/disable streaming output in rich panel
            (default: "false")
        CYBERSENTRY_TELEMETRY: Enable/disable telemetry (default: "true")
        CYBERSENTRY_PARALLEL: Number of parallel agent instances to run
        ...
        CYBERSENTRY_GUARDRAILS: Enable/disable security guardrails for agents
    ...
    Extensions (only applicable if the right extension is installed):
        ...
        "report"
            CYBERSENTRY_REPORT: Enable/disable reporter mode. Possible values:
...
Usage Examples:
    ...
    CTF_NAME="kiddoctf" CTF_CHALLENGE="02 linux ii" \
        CYBERSENTRY_AGENT_TYPE="one_tool_agent" CYBERSENTRY_MODEL="cybe4sent1nel0" \
        CYBERSENTRY_TRACING="false" cybersentry

    # Run a harder CTF
    CTF_NAME="hackableii" CYBERSENTRY_AGENT_TYPE="redteam_agent" \
        CTF_INSIDE="False" CYBERSENTRY_MODEL="cybe4sent1nel0" \
        CYBERSENTRY_TRACING="false" cybersentry

    # Run without a target in human-in-the-loop mode, generating a report
    CYBERSENTRY_TRACING=False CYBERSENTRY_REPORT=pentesting CYBERSENTRY_MODEL="cybe4sent1nel0" \
        cybersentry

    # Run with online episodic memory
    #   registers memory every 5 turns:
    #   limits the cost to 5 dollars
    CTF_NAME="hackableII" CYBERSENTRY_MEMORY="episodic" \
        CYBERSENTRY_MODEL="cybe4sent1nel0" CYBERSENTRY_MEMORY_ONLINE="True" \
        CTF_INSIDE="False" CTF_HINTS="False"  \
        CYBERSENTRY_PRICE_LIMIT="5" cybersentry

    # Run with custom long_term_memory interval
    # Executes memory long_term_memory every 3 turns:
    CTF_NAME="hackableII" CYBERSENTRY_MEMORY="episodic" \
        CYBERSENTRY_MODEL="cybe4sent1nel0" CYBERSENTRY_MEMORY_ONLINE_INTERVAL="3" \
        CYBERSENTRY_MEMORY_ONLINE="False" CTF_INSIDE="False" \
        CTF_HINTS="False" cybersentry

    # Run with parallel agents (3 instances)
    CTF_NAME="hackableII" CYBERSENTRY_AGENT_TYPE="redteam_agent" \
        CYBERSENTRY_MODEL="cybe4sent1nel0" CYBERSENTRY_PARALLEL="3" cybersentry
"""
...
from cybersentry.sdk.agents.stream_events import RunItemStreamEvent
...
from cybersentry.sdk.agents.models.openai_chatcompletions import (
    get_agent_message_history,
    get_all_agent_histories,
)
from cybersentry.sdk.agents.exceptions import OutputGuardrailTripwireTriggered, InputGuardrailTripwireTriggered
from cybersentry.sdk.agents.items import ToolCallOutputItem
from cybersentry.sdk.agents import Agent, OpenAIChatCompletionsModel, Runner, set_tracing_disabled
...
from cybersentry.repl.commands.parallel import PARALLEL_CONFIGS, ParallelConfig, PARALLEL_AGENT_INSTANCES
from cybersentry.repl.commands import FuzzyCommandCompleter, handle_command as commands_handle_command
from cybersentry.internal.components.metrics import process_metrics
...
def custom_warning_handler(message, category, filename,
                           lineno, file=None, line=None):
    # Only show warnings in debug mode
    if os.getenv("CYBERSENTRY_DEBUG", "1") == "2":
...
# Suppress ALL warnings in production mode (unless CYBERSENTRY_DEBUG=2)
if os.getenv("CYBERSENTRY_DEBUG", "1") != "2":
...
class ComprehensiveErrorFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if "sse" in msg and any(word in msg for word in [
                                "cleanup", "closing", "shutdown", "closed"]):
            return False
...
# CYBERSENTRY agents and metrics imports

# CYBERSENTRY REPL imports
...
def update_agent_models_recursively(agent, new_model, visited=None):
    ...
    if hasattr(agent, "model") and hasattr(agent.model, "model"):
        ...
        # This ensures the model doesn't have stale state from the previous
        # model
...
def run_cybersentry_cli(
    ...
    """
    Run a simple interactive CLI loop for CYBERSENTRY.
    ...
    while True:
        ...
        if previous_ctf_name != os.getenv("CTF_NAME", None):
            if is_pentestperf_available():
                ...
                previous_ctf_name = os.getenv("CTF_NAME", None)
                ctf_init = 0
        # Check if CYBERSENTRY_MAX_TURNS has been updated via /config
        ...
        if current_max_turns != str(prev_max_turns):
            ...
            if turn_limit_reached and turn_count < max_turns:
                turn_limit_reached = False
                console.print(
                    "[green]Turn limit increased. You can now continue using CYBERSENTRY.[/green]"
                )
        ...
        if turn_count >= max_turns and max_turns != float("inf"):
            if not turn_limit_reached:
                ...
                console.print(
                    f"[bold red]Error: Maximum turn limit ({
                        int(max_turns)}) reached.[/bold red]"
                )
                console.print(
                    "[yellow]You must increase the limit using the /config command: /config CYBERSENTRY_MAX_TURNS=<new_value>[/yellow]"
                )
                console.print(
        ...
        try:
            ...
            if current_agent_type != last_agent_type:
                ...
                if os.environ.get("CYBERSENTRY_AGENT_SWITCH_HANDLED") == "1":
                    os.environ["CYBERSENTRY_AGENT_SWITCH_HANDLED"] = "0"  # Reset flag
                    ...
                    if agent:
                        ...
                        # If the agent is None (weak reference expired),
                        # recreate it
                        agent = get_agent_by_name(
                            current_agent_type, agent_id="P1")
                        last_agent_type = current_agent_type
                        # Re-register with AGENT_MANAGER
                        agent_name = agent.name if hasattr(
                            agent, "name") else current_agent_type
                        AGENT_MANAGER.set_active_agent(agent, agent_name, "P1")
                    continue

                try:
                    # CRITICAL: Set up history transfer BEFORE creating the new
                    # agent
                    from cybersentry.sdk.agents.simple_agent_manager import AGENT_MANAGER
                    ...
                    if hasattr(agent, "name"):
                        ...
                        current_history = AGENT_MANAGER.get_message_history(
                            current_agent_name)
                        if current_history:
                            AGENT_MANAGER._pending_history_transfer = list(
                                current_history)  # Make a copy
                    ...
                    # IMPORTANT: Always use the agent's proper name, not the
                    # agent key
                    agent_name = agent.name if hasattr(
                        agent, "name") else current_agent_type
                    ...
                    # This ensures the model has its own history from
                    # AGENT_MANAGER
                    if hasattr(agent, "model") and hasattr(
                            agent.model, "message_history"):
                        agent_history = AGENT_MANAGER.get_message_history(
                            agent_name)
                    ...
                    if hasattr(agent, "model"):
                        ...
                        # Check for agent-specific model override
                        agent_specific_model = os.getenv(
                            f"CYBERSENTRY_{current_agent_type.upper()}_MODEL")
                        model_to_apply = (
                        ...
                        if hasattr(agent.model, "set_agent_name"):
                            agent.model.set_agent_name(
                                get_agent_short_name(agent))

                        # Clear any asyncio tasks that might be lingering from the previous agent
                        # This helps prevent event loop issues after agent
                        # switching
                        try:
                            # Get all running tasks
                            all_tasks = asyncio.all_tasks() if hasattr(
                                asyncio, 'all_tasks') else asyncio.Task.all_tasks()
                            # Cancel tasks that aren't the current task
                            current_task = asyncio.current_task() if hasattr(
                                asyncio, 'current_task') else asyncio.Task.current_task()
            ...
            if not user_input.strip():
                # Set a default message to continue the conversation
            ...
            try:
                if PARALLEL_CONFIGS and PARALLEL_ISOLATION.is_parallel_mode():
                    ...
                    for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                        ...
                        if instance_key in PARALLEL_AGENT_INSTANCES:
                            ...
                            if hasattr(instance_agent, 'model') and hasattr(
                                ...
                                # via the add_to_message_history method, but
                                # let's make sure
                    ...
                    if saved_count > 0:
                        # Sync isolated histories with AGENT_MANAGER for
                        # display
                        for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                            agent_id = config.id or f"P{idx}"
                            isolated_history = PARALLEL_ISOLATION.get_isolated_history(
                                agent_id)
                            if isolated_history:
                                # Get the agent display name
                                from cybersentry.agents import get_available_agents
                                ...
                                if config.agent_name in available_agents:
                                    ...
                                    # Clear and replace the history in
                                    # AGENT_MANAGER
                                    AGENT_MANAGER.clear_history(
                                        agent_display_name)
                                    for msg in isolated_history:
                                        AGENT_MANAGER.add_to_history(
                                            agent_display_name, msg)
            except Exception as e:
                # Only log this error in debug mode
                logger = logging.getLogger(__name__)
                logger.debug(
                    f"Error saving parallel agents' histories: {
                        str(e)}")

            # Clean up any pending tool calls before exiting
            try:
                # Access the converter directly to clean up any pending tool calls
                # converter is instance-based, access via
                # agent.model._converter

                # Check if any tool calls are pending (have been issued but
                ...
                if hasattr(agent.model, "_converter") and hasattr(
                    ...
                    for call_id, call_info in list(
                        ...
                        # Check if this tool call has a corresponding response
                        # in message_history
                        ...
                        if not tool_response_exists:
                            # First ensure there's a matching assistant message
                            # with this tool call
                            ...
                            pending_calls.append(
                                call_info.get("name", "unknown"))

                # Apply message list fixes to ensure consistency
                if pending_calls:
                    from cybersentry.util import fix_message_list

                    agent.model.message_history[:] = fix_message_list(
                        agent.model.message_history)
            except Exception:
                pass

            try:
                # Get more accurate active and idle time measurements from the
                # timer functions
                from cybersentry.util import COST_TRACKER, get_active_time_seconds, get_idle_time_seconds
                ...
                metrics = {
                    ...
                    # Using active time as LLM time
                    "llm_time": format_time(active_time_seconds),
                    "llm_percentage": round((active_time_seconds / Total) * 100, 1)
                    if Total > 0
                    else 0.0,
                    # Add formatted session cost
                    "session_cost": f"${session_cost:.6f}",
                }
                logging_path = (
                    session_logger.filename if hasattr(
                        session_logger, "filename") else None
                )
                ...
                # env. variable CYBERSENTRY_TELEMETRY and there's internet connectivity
                telemetry_enabled = os.getenv(
                    "CYBERSENTRY_TELEMETRY", "true").lower() != "false"
                if (
                    telemetry_enabled
                ...
                GLOBAL_USAGE_TRACKER.end_session(
                    final_cost=COST_TRACKER.session_total_cost)
                ...
                # Prevent duplicate cost display from the COST_TRACKER exit
                # handler
                os.environ["CYBERSENTRY_COST_DISPLAYED"] = "true"
        ...
        try:
            ...
            if (
                ...
                console.print(
                    "[yellow]Please use /config to increase CYBERSENTRY_MAX_TURNS limit.[/yellow]"
                )
            ...
            if (
                ...
                agents_with_prompts = [
                    (idx, config) for idx, config in enumerate(
                        PARALLEL_CONFIGS, 1) if config.prompt]

                # First ensure ALL parallel configs have agent instances (not just selected ones)
                # This prevents agents from disappearing from history when not
                # selected
                from cybersentry.agents import get_available_agents
                ...
                # Get agent IDs
                agent_ids = [
                    config.id or f"P{idx}" for idx,
                    config in enumerate(
                        PARALLEL_CONFIGS,
                        1)]

                # Check if we already have isolated histories (e.g., from /load parallel)
                # If not, transfer the current agent's history to all parallel
                # agents
                already_has_histories = False
                if PARALLEL_ISOLATION.is_parallel_mode():
                    # Check if at least one agent has a non-empty isolated
                    # history
                ...
                if not already_has_histories:
                    ...
                    # Pattern 17 (Red/Blue team with different contexts) should
                    # only transfer to P1
                    transfer_to_all = True

                    # Check if this is a pattern that requires different contexts
                    # This is typically pattern 17 or similar patterns with
                    # "different contexts" in the description
                    pattern_description = os.getenv(
                        "CYBERSENTRY_PATTERN_DESCRIPTION", "")
                    if "different contexts" in pattern_description.lower():
                        transfer_to_all = False

                    if transfer_to_all:
                        # Transfer to parallel mode - creates isolated copies
                        # for each agent
                    ...
                    # Already have isolated histories, just ensure we're in
                    # parallel mode
                    PARALLEL_ISOLATION._parallel_mode = True
                ...
                for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                    ...
                    if instance_key not in PARALLEL_AGENT_INSTANCES:
                        ...
                        if base_agent:
                            agent_display_name = getattr(
                                base_agent, "name", config.agent_name)
                            custom_name = f"{agent_display_name} #{idx}"

                            # Determine model
                            model_to_use = config.model or os.getenv(
                                "CYBERSENTRY_MODEL", "cybe4sent1nel0")

                            # Create and store the instance
                            # No shared_message_history - each agent gets its
                            # own isolated copy
                ...
                # Each agent will get its own isolated history to prevent
                # mixing
                ...
                async def run_agent_instance(
                    ...
                    try:
                        ...
                        # Use all PARALLEL_CONFIGS to ensure consistent
                        # numbering
                        instance_number = PARALLEL_CONFIGS.index(config) + 1
                        agent_id = config.id or f"P{instance_number}"

                        # Get the existing instance from
                        # PARALLEL_AGENT_INSTANCES
                        instance_key = (config.agent_name, instance_number)
                        instance_agent = PARALLEL_AGENT_INSTANCES.get(
                            instance_key)

                        if not instance_agent:
                            # Fallback: create instance if not found (shouldn't
                            # happen normally)
                            from cybersentry.agents import get_available_agents
                            from cybersentry.agents.patterns import get_pattern
                            ...
                            if config.agent_name.endswith("_pattern"):
                                ...
                                if pattern and hasattr(pattern, 'entry_agent'):
                                    ...
                                    # For patterns, we create the pattern
                                    # itself, not individual agents
                                    actual_agent_name = config.agent_name
                            else:
                                base_agent = get_available_agents().get(config.agent_name.lower())
                            ...
                            # since they already have unique names like "Red
                            # team manager"
                            ...
                            # Each agent gets its own isolated history from
                            # PARALLEL_ISOLATION
                            instance_agent = get_agent_by_name(
                            ...
                            # Store a strong reference to prevent garbage
                            # collection
                            PARALLEL_AGENT_INSTANCES[instance_key] = instance_agent
                        ...
                        # Ensure the model is properly set for the agent and
                        # all handoff agents
                        model_to_use = config.model or os.getenv(
                            "CYBERSENTRY_MODEL", "cybe4sent1nel0")
                        if model_to_use:
                        ...
                        if config.prompt:
                            # Use the custom prompt instead of regular user
                            # input
                        ...
                        # Clean up any streaming resources created by this
                        # agent's tools
                        try:
                            from cybersentry.util import finish_tool_streaming, cli_print_tool_output, _LIVE_STREAMING_PANELS

                            # In parallel mode, we need to update the final
                            # status of panels
                            if hasattr(cli_print_tool_output,
                                       "_streaming_sessions"):
                                agent_display_name = getattr(
                                    instance_agent, 'name', config.agent_name)
                        ...
                        # Clean up any streaming resources before propagating
                        # cancellation
                        try:
                            from cybersentry.util import cleanup_agent_streaming_resources

                            # Clean up streaming sessions for this specific
                            # agent
                            if instance_agent:
                                agent_display_name = getattr(
                                    instance_agent, 'name', config.agent_name)
                        ...
                        # Save the agent's history before propagating the
                        # cancellation
                        if instance_agent and agent_id:
                            if hasattr(instance_agent, 'model') and hasattr(
                                    instance_agent.model, 'message_history'):
                        ...
                        # Clean up any streaming resources before handling
                        # exception
                        try:
                            from cybersentry.util import cleanup_agent_streaming_resources
                ...
                try:
                    ...
                    # Force save all parallel agent histories to
                    # PARALLEL_ISOLATION
                    for idx, config in enumerate(PARALLEL_CONFIGS, 1):
                        instance_key = (config.agent_name, idx)
                        if instance_key in PARALLEL_AGENT_INSTANCES:
            ...
            if user_input.startswith("/") or user_input.startswith("$"):
                ...
                # If command wasn't recognized, show error (skip for /shell or
                # /s)
                if command not in ("/shell", "/s"):
                    console.print(
                        f"[red]Command failed or unknown: {command}[/red]")
                continue
            from rich.text import Text
            ...
            if hasattr(agent, 'model') and hasattr(
                ...
                for msg in agent.model.message_history:
                    ...
                    if role == "user":
                        ...
                        history_context.append(
                            {"role": "system", "content": content or ""})
                    elif role == "assistant":
                        if tool_calls:
                            history_context.append(
                                {
            ...
            # Fix message list structure BEFORE sending to the model to prevent
            # errors
            try:
                from cybersentry.util import fix_message_list
            ...
            # Process the conversation with the agent - with parallel execution
            # if enabled
            if parallel_count > 1:
                # Parallel execution mode (always non-streaming)
                async def run_agent_instance(
                        instance_number, conversation_context):
                    """Run a single agent instance with its own complete context"""
                    try:
                        # Create a fresh agent instance with unique name to
                        ...
                        custom_name = f"{agent_display_name} #{
                            instance_number + 1}"
                        instance_agent = get_agent_by_name(
                            last_agent_type, custom_name=custom_name, agent_id=f"P{
                                instance_number + 1}")
                        ...
                        if hasattr(instance_agent, "model") and hasattr(
                            ...
                            if hasattr(instance_agent.model, "model") and hasattr(
                                ...
                                instance_specific_model = os.getenv(
                                    f"CYBERSENTRY_{
                                        last_agent_type.upper()}_{
                                        instance_number + 1}_MODEL"
                                )

                                if instance_specific_model:
                                    # Use instance-specific model (e.g.,
                                    # CYBERSENTRY_BUG_BOUNTER_1_MODEL)
                        ...
                        logger = logging.getLogger(__name__)
                        logger.error(
                            f"Error in instance {instance_number}: {
                                str(e)}", exc_info=True)

                        # Only show error in debug mode
                        if os.getenv("CYBERSENTRY_DEBUG", "1") == "2":
                            console.print(
                                f"[bold red]Error in instance {instance_number}: {
                                    str(e)}[/bold red]"
                            )
                ...
                for idx, result in results:
                    if result and hasattr(
                            result, "final_output") and result.final_output:
                ...
                cybersentry_stream = os.getenv("CYBERSENTRY_STREAM", "false")
                # Handle empty string or None values
                if not cybersentry_stream or cybersentry_stream.strip() == "":
                    cybersentry_stream = "false"
                stream = cybersentry_stream.lower() == "true"

                # Single agent execution (original behavior)
                if stream:

                    async def process_streamed_response(
                            agent, conversation_input):
                        ...
                        try:
                            ...
                            # Consume events so the async generator is
                            # executed.
                                ...
                                if isinstance(event, RunItemStreamEvent):
                                    if event.name == "tool_called":
                                        ...
                                        if hasattr(event.item, 'raw_item'):
                                            ...
                                            # which has a direct call_id
                                            # attribute
                                            call_id = getattr(
                                                event.item.raw_item, 'call_id', None)
                                            if call_id:
                                                tool_calls_seen[call_id] = event.item
                                    elif event.name == "tool_output":
                                        # Ensure item is a ToolCallOutputItem
                            ...
                            # Clean up the result object if it has cleanup
                            # methods
                            ...
                            try:
                                ...
                                # Silently ignore cleanup errors during
                                # interrupt
                                pass
                            ...
                            logger = logging.getLogger(__name__)
                            logger.error(
                                f"Error occurred during streaming: {
                                    str(e)}", exc_info=True)
                    ...
                    try:
                        ...
                        print(
                            f"\033[93mThe agent's output was blocked for security reasons.\033[0m")
                        print(
                            f"\033[96mYou can continue the conversation with a different request.\033[0m\n")
                        ...
                        if "This event loop is already running" in str(
                                e) or "Cannot close a running event loop" in str(e):
                            ...
                            try:
                                ...
                                # Display a user-friendly warning instead of
                                # crashing (new event loop)
                                guardrail_name = e.guardrail_result.guardrail.get_name()
                                reason = e.guardrail_result.output.output_info.get(
                                    "reason", "Security policy violation")
                                ...
                                print(
                                    f"\n\033[91m🛡️  SECURITY GUARDRAIL TRIGGERED\033[0m")
                                print(
                                    f"\033[91mGuardrail: {guardrail_name}\033[0m")
                                print(f"\033[91mReason: {reason}\033[0m")
                                print(
                                    f"\033[93mThe agent's output was blocked for security reasons.\033[0m")
                                print(
                                ...
                                # Close the loop and continue the conversation
                                # loop
                                new_loop.close()
                                continue
                            finally:
                    ...
                    try:
                        ...
                        print(f"\033[91mReason: {reason}\033[0m")
                        print(
                            f"\033[93mYour input was blocked for security reasons.\033[0m")
                        ...
                        if "base64" in reason.lower() or "pattern" in reason.lower():
                            ...
                            print(
                                f"  1. Type \033[92m/clear\033[0m to clear the conversation history")
                            print(
                                f"  2. Type \033[92m/config set 26 false\033[0m to temporarily disable guardrails")
                            print(
                                f"  3. Type \033[92m/exit\033[0m to exit CYBERSENTRY")
                        else:
                            print(
                                f"\033[96mPlease rephrase your request or try a different approach.\033[0m\n")
                    ...
                    # Los tool calls (assistant messages) ya se añaden
                    # correctamente en openai_chatcompletions.py
                    for item in response.new_items:
                        # Handle ONLY tool call output items (tool results)
                        if isinstance(item, ToolCallOutputItem):
                            tool_call_id = item.raw_item["call_id"]

                            # Verificar si ya existe este tool output en
                            # message_history para evitar duplicación
                            tool_msg_exists = any(
                                msg.get("role") == "tool"
                ...
                # Ensure every tool message has a preceding assistant message
            ...
            try:
                from cybersentry.util import cleanup_all_streaming_resources
                ...
                for msg in agent.model.message_history:
                    if msg.get("role") == "assistant" and msg.get(
                        ...
                        for tool_call in msg["tool_calls"]:
                            ...
                            if call_id:
                                # Check if this tool call has a corresponding
                                # tool result
                                has_result = any(
                                    m.get("role") == "tool" and m.get(
                                        "tool_call_id") == call_id
            ...
            # Add a small delay to allow the system to settle after
            # interruption
            import time
            time.sleep(0.1)
            ...
            try:
                ...
                if loop and loop.is_running():
                    # Can't close a running loop, but we can clear pending
                    # tasks
                    pending = asyncio.all_tasks(loop) if hasattr(
                        asyncio, 'all_tasks') else asyncio.Task.all_tasks(loop)
                    for task in pending:
                        task.cancel()
            except Exception:
                pass
...
def create_last_log_symlink(log_filename):
    ...
    try:
        ...
        # Create new symlink pointing to just the filename (relative path
        # within logs dir)
...
def main():
    ...
    run_cybersentry_cli(agent, initial_prompt=initial_prompt)
