...
def _load_component(name: str) -> Any:
    ...
    try:
        ...
        if sys.modules.get(f"cybersentry.internal.components.{name}"):
            return sys.modules[f"cybersentry.internal.components.{name}"]
        return importlib.import_module(f"cybersentry.internal.components.{name}")
    except:
        return None
