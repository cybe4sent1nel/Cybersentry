...
import re
import unicodedata
from cybersentry.tools.common import (run_command, run_command_async,
                              list_shell_sessions,
                              get_session_output,
...
async def generic_linux_command(command: str = "",
    ...
    Environment Detection:
        ...
        - Container: Commands run in Docker containers when CYBERSENTRY_ACTIVE_CONTAINER is set
    ...
    if command.strip() == "env info" or command.strip() == "environment info":
        ...
        # Check workspace
        try:
            from cybersentry.tools.common import _get_workspace_dir
    ...
    guardrails_enabled = os.getenv("CYBERSENTRY_GUARDRAILS", "true").lower() != "false"
    ...
    # Tools always stream EXCEPT in parallel mode or when CYBERSENTRY_STREAM=False
    ...
    # Check if CYBERSENTRY_STREAM is explicitly set to False
    if os.getenv("CYBERSENTRY_STREAM", "true").lower() == "false":
        stream = False
    
    # Simple heuristic: If CYBERSENTRY_PARALLEL > 1 AND we have a P agent ID, disable streaming
    ...
    # Respect CYBERSENTRY_GUARDRAILS environment variable
    guardrails_enabled = os.getenv("CYBERSENTRY_GUARDRAILS", "true").lower() != "false"
    ...
    # Enhanced sanitization for curl/wget responses - respect CYBERSENTRY_GUARDRAILS setting
    guardrails_enabled = os.getenv("CYBERSENTRY_GUARDRAILS", "true").lower() != "false"
