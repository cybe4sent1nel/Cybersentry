...
from cybersentry.repl.commands.base import (
    Command, COMMANDS, COMMAND_CYBERSENTRY, 
...
class TestCommand:
    ...
    def setup_and_cleanup(self):
        ...
        self.original_cybe4sent1neles = COMMAND_CYBERSENTRY.copy()
        ...
        COMMAND_CYBERSENTRY.clear()
        COMMAND_CYBERSENTRY.update(self.original_cybe4sent1neles)
    ...
    def test_command_initialization(self, sample_command):
        ...
        assert sample_command.cybe4sent1neles == ["/t", "/test-cmd"]
        assert isinstance(sample_command.subcommands, dict)
        assert len(sample_command.subcommands) == 0
    
    def test_command_initialization_without_cybe4sent1neles(self):
        """Test Command initialization without cybe4sent1neles."""
        cmd = Command("/test", "Test command")
        assert cmd.name == "/test"
        assert cmd.description == "Test command"
        assert cmd.cybe4sent1neles == []
...
class TestCommandRegistry:
    ...
    def test_register_command(self, test_commands):
        ...
        assert "/t1" in COMMAND_CYBERSENTRY
        assert COMMAND_CYBERSENTRY["/t1"] == cmd.name
    ...
    def test_register_multiple_commands(self, test_commands):
        ...
        # Check cybe4sent1neles
        assert COMMAND_CYBERSENTRY["/t1"] == "/test1"
        assert COMMAND_CYBERSENTRY["/t2"] == "/test2"
        assert COMMAND_CYBERSENTRY["/test-two"] == "/test2"
    ...
    def test_get_command_by_cybe4sent1nel(self, test_commands):
        """Test getting a command by its cybe4sent1nel."""
        cmd = test_commands[0]
        register_command(cmd)
    ...
    def test_handle_command_by_cybe4sent1nel(self, test_commands):
        """Test handling a command by its cybe4sent1nel."""
        cmd = test_commands[0]
        cmd.handle = Mock(return_value=True)
        register_command(cmd)
...
class TestCustomCommand:
    ...
    def test_custom_command_with_subcommands(self):
        ...
        result2 = handle_command("/m", ["stop"])  # Test cybe4sent1nel
        assert result2 is True
...
class TestCommandIntegration:
    ...
    def test_complete_command_lifecycle(self):
        ...
        assert len(COMMAND_CYBERSENTRY) == 3
        
        # Test various command executions
        assert handle_command("/cmd1", ["action"]) is True
        assert cmd1.action_count == 1
        
        assert handle_command("/c1", ["action"]) is True  # Using cybe4sent1nel
        assert cmd1.action_count == 2
        
        assert handle_command("/cmd2") is True
        assert cmd2.call_count == 1
        
        assert handle_command("/c2") is True  # Using cybe4sent1nel
        assert cmd2.call_count == 2
        
        assert handle_command("/second") is True  # Using second cybe4sent1nel
        assert cmd2.call_count == 3
        
        # Test non-existent command
        assert handle_command("/nonexistent") is False
    
    def test_command_cybe4sent1nel_conflicts(self):
        """Test handling of cybe4sent1nel conflicts (last registered wins)."""
        cmd1 = Command("/cmd1", "First command", ["/shared"])
        cmd2 = Command("/cmd2", "Second command", ["/shared"])  # Same cybe4sent1nel
        
        register_command(cmd1)
        register_command(cmd2)  # This should overwrite the cybe4sent1nel
        
        # The cybe4sent1nel should point to the last registered command
        assert COMMAND_CYBERSENTRY["/shared"] == "/cmd2"
