...
Usage:
    ...
    cybersentry-replay path/to/file.jsonl 0.5
...
Or alternatively:
    asciinema rec --command="JSONL_FILE_PATH='cybersentryextensions-memory/cybersentryextensions/memory/it/pentestperf/hackableii/hackableII_autonomo.jsonl' REPLAY_DELAY='0.05' cybersentry-replay"
...
os.environ["Cybersentry_DISABLE_SESSION_RECORDING"] = "true"

# Add the parent directory to the path to import cybersentry modules
...
from cybersentry.sdk.agents.run_to_jsonl import get_token_stats, load_history_from_jsonl
from cybersentry.repl.ui.banner import display_banner
...
def replay_conversation(messages: List[Dict], replay_delay: float = 0.5, usage: Tuple = None, jsonl_file_path: str = None, full_data: List[Dict] = None) -> None:
    ...
    for i, message in enumerate(messages):
        try:
            ...
            if role == "user":
                print(color(f"Cybersentry> ", fg="cyan") + f"{content}")
                turn_counter += 1
